...
describe("/northcoders-news", () => {
  ...
  let commentDocs;
  beforeEach(() => {
    return seedDB({ topicData, userData, articleData, commentData }).then(
      docs => {
  ...
  describe("/topics", () => {
    it("GET returns an array of all topics and with a status 200", () => {
      return request
        .get("/api/topics")
        .expect(200)
        .then(res => {
    ...
    it("GET returns status 200 and the profile data for a specific user", () => {
      return request
        ...
        .then(res => {
          ...
          expect(res.body.user.username).to.eql("dedekind561");
        });
    });
  ...
  describe("/comments", () => {
    ...
    it("PUT sends back status 201 and increments the vote count of comment", () => {
      return request
        .put(`/api/comments/${commentDocs[0]._id}?vote=up`)
        .expect(201)
        .then(res => {
          expect(res.body.votes).to.eql(8);
        });
    });
    it("PUT sends back status 201 and decrements the vote count of comment", () => {
      return request
        .put(`/api/comments/${commentDocs[0]._id}?vote=down`)
        .expect(201)
        .then(res => {
          expect(res.body.votes).to.eql(6);
        });
    });
    it("returns a 400 with an error message where the vote query is invalid", () => {
      return request
        .put(`/api/comments/${commentDocs[0]._id}?vote=hello`)
        .expect(400)
        .then(res => {
          expect(res.body).to.eql({ message: ` 400 - Bad Request` });
        });
